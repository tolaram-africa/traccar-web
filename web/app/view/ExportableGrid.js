            ...
            if (!this.store.isGrouped()) {
                ...
                if (tpl != null) {
                    storeGroups.each(function (gr) {
                        var renderedGroupValue = null;
                        var groupColumn = null;
                        var groupField = this.store.getGroupField() || this.store.getGrouper() != null ? this.store.getGrouper().getProperty() : null;
                        if (groupField != null) {
                            groupColumn = this.view.getVisibleColumnManager().getHeaderByDataIndex(groupField);
                            renderedGroupValue = groupColumn ? groupColumn.config.renderer ? groupColumn.config.renderer(gr.getAt(0).get(groupField), {}, gr.getAt(0)) : gr.getGroupKey() : gr.getAt(0).get(groupField);
                        } else if (this.store.getGrouper() && this.store.getGrouper().getGroupFn()) {
                            renderedGroupValue = this.store.getGrouper().getGroupFn().call(this.store.getGrouper(), gr.getAt(0));
                        }
                        var groupingHeaderValue = tpl.apply({
                            groupValue: gr.getGroupKey(),
                            groupField: groupField,
                            columnName: groupColumn ? groupColumn.text : groupField,
                            ...
                            for (var i = 0; i < exportTask.exportableColumns.length; i++) {
                                ...
                                if (exCol) {
                                    ...
                                    if (exCol.groupingSummaryConverter) {
                                        modelData[exCol.dataIndex] = exCol.groupingSummaryConverter(summaryVal, features['groupingsummary'].summaryData[gr.getGroupKey()], exCol.dataIndex, grSummaryCellValues);
                                    } else if (exCol.summaryRenderer) {
                                        modelData[exCol.dataIndex] = exCol.summaryRenderer(summaryVal, features['groupingsummary'].summaryData[gr.getGroupKey()], exCol.dataIndex, grSummaryCellValues);
                                    } else {
            ...
            if (features['summary']) {
                ...
                for (var i = 0; i < exportTask.exportableColumns.length; i++) {
                    ...
                    if (exCol) {
                        ...
                        var summaryCellValues = this.lockable ? exCol.locked ? this.lockedGrid.view.cellValues : this.normalGrid.view.cellValues : this.view.cellValues;
                        summaryCellValues.record = features['summary'].summaryRecord;
                        summaryCellValues.column = exCol;
                        if (exCol.summaryConverter) {
                            modelData[exCol.dataIndex] = exCol.summaryConverter(summaryVal, features['summary'].summaryData, exCol.dataIndex, summaryCellValues);
                        } else if (exCol.summaryRenderer) {
                            modelData[exCol.dataIndex] = exCol.summaryRenderer(summaryVal, features['summary'].summaryData, exCol.dataIndex, summaryCellValues);
                        } else {
                            modelData[exCol.dataIndex] = summaryVal;
                        }
                    }
